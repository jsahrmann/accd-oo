...
dogs_final <- dogs_a14[dogs_i10, on = c("id", "sn")]
...
dogs_analysis <-
  dogs_final[
    dogs_outcomeAndCensoringDates[,
      .(id, oo_event, oo_t2e, obese_event, obese_t2e)],
    on = "id"
  ][,
    ageCat :=
      cut(
        ageDays,
        breaks = c(0, 180, 365, 730, Inf),
        labels =
          c("Birth to 6 mo", "6 mo to 1 y", "1 y to 2 y", ">2 y"),
        include.lowest = TRUE
      )
  ]


xtab_sexBySN <- dogs_analysis %$%
  table(sex, sn, useNA = "ifany")
xtab_sexBySN
proportions(xtab_sexBySN, margin = 2)

xtab_mixed_breedBySN <- dogs_analysis %$%
  table(mixed_breed, sn, useNA = "ifany")
xtab_mixed_breedBySN
proportions(xtab_mixed_breedBySN, margin = 2)

xtab_wellness_planBySN <- dogs_analysis %$%
  table(wellness_plan, sn, useNA = "ifany")
xtab_wellness_planBySN
proportions(xtab_wellness_planBySN, margin = 2)

qntl_age <- dogs_analysis %$%
  tapply(ageYears, sn, \(.x) quantile(.x))
qntl_age

xtab_ageCatBySN <- dogs_analysis %$%
  table(ageCat, sn, useNA = "ifany")
xtab_ageCatBySN
proportions(xtab_ageCatBySN, margin = 2)

qntl_weight <- dogs_analysis %$%
  tapply(weight, sn, \(.x) quantile(.x))
qntl_weight

xtab_sizeBySN <- dogs_analysis %$%
  table(size, sn, useNA = "ifany")
xtab_sizeBySN
proportions(xtab_sizeBySN, margin = 2)

xtab_ooBySN <- dogs_analysis %$%
  table(oo_event, sn, useNA = "ifany")
xtab_ooBySN
proportions(xtab_ooBySN, margin = 2)

qntl_oo_t2e <- dogs_analysis %$%
  tapply(oo_t2e, sn, \(.x) quantile(.x))
qntl_oo_t2e

xtab_obeseBySN <- dogs_analysis %$%
  table(obese_event, sn, useNA = "ifany")
xtab_obeseBySN
proportions(xtab_obeseBySN, margin = 2)

qntl_obese_t2e <- dogs_analysis %$%
  tapply(obese_t2e, sn, \(.x) quantile(.x))
qntl_obese_t2e
...
dogs_analysis[,
  `:=`(
    sn = factor(sn, levels = 0:1, labels = c("Intact", "S/N")),
    sex = factor(sex, levels = c("Female", "Male")),
    mixed_breed = factor(mixed_breed, levels = c("N", "Y")),
    wellness_plan =
      factor(wellness_plan, levels = 0:1, labels = c("N", "Y")),
    size =
      factor(
        size,
        levels =
          c("Toy and Small", "Standard", "Medium", "Large", "Giant")
      )
  )
]
...
p1 <- dogs_analysis %>%
  ggplot(aes(x = ageYears, colour = sn, fill = sn)) +
...
cairo_pdf(
  "./fig/bar_ageAtIndex_20220314.pdf", width = 7, height = 9)
p1 / p2
dev.off()
...
p1 <- dogs_analysis %>%
  ggplot(aes(x = weight, colour = sn, fill = sn)) +
  geom_density(alpha = 0.25, position = "identity") +
  scale_colour_manual("", values = c(col0, col1)) +
  scale_fill_manual("", values = c(col0, col1)) +
  xlab("Weight at Spay/Neuter or First Visit (lbs.)") +
  ylab("Density") +
  theme(
    legend.position = "bottom")

y <- as_tibble(proportions(xtab_sizeBySN, margin = 2)) %>%
  mutate(
    sn = factor(sn, levels = c("Intact", "S/N")),
    size =
      factor(
        size,
        levels =
...
p2 <- y %>%
  ggplot(aes(x = size, y = n, colour = sn, fill = sn)) +
  geom_col(alpha = 0.25, position = "dodge") +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_colour_manual("", values = c(col0, col1)) +
  scale_fill_manual("", values = c(col0, col1)) +
  xlab("Breed Size Category") +
  ylab("Percentage") +
  theme(
    legend.position = "bottom")

cairo_pdf(
  "./fig/bar_weightAndSize_20220314.pdf", width = 7, height = 9)
p1 / p2
dev.off()
...
kmEstOO <-
  survfit(
    Surv(oo_t2e, oo_event) ~ sn, data = dogs_analysis, conf.int = .99)
kmEstObese <-
  survfit(
    Surv(obese_t2e, obese_event) ~ sn,
    data = dogs_analysis, conf.int = .99)

p1 <- kmEstOO %>%
  ggsurv(
    CI = TRUE, plot.cens = FALSE, surv.col = c(col0, col1), lty.ci = 3
  ) +
  labs(colour = "Spay/Neuter Status", linetype = "Spay/Neuter Status") +
  xlab("Days between Index and O/O Event or Censoring") +
  ylab("Proportion of Dogs without O/O Event") +
  theme(
    legend.position = "bottom",
...
p2 <- kmEstObese %>%
  ...
  labs(colour = "Spay/Neuter Status", linetype = "Spay/Neuter Status") +
  xlab("Days between Index and Obese Event or Censoring") +
  ylab("Proportion of Dogs without Obese Event") +
  theme(
    legend.position = "bottom",
    legend.title = element_blank())

cairo_pdf("./fig/kmcurve_20220314.pdf", width = 7, height = 9)
p1 / p2
dev.off()


pdftools::pdf_combine(
  c("./doc/table1_20220314.pdf",
    "./fig/bar_ageAtIndex_20220314.pdf",
    "./fig/bar_weightAndSize_20220314.pdf",
    "./fig/kmcurve_20220314.pdf"),
  "./fig/Descriptive analysis of 2014 Banfield cohort 20220314.pdf")
